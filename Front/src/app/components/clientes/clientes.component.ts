...
export class ClientesComponent implements OnInit {
  ...
  cargar() { 
    this.cargando = true; this.svc.listar().subscribe(
      { next: d => { this.lista = d as Cliente[]; 
        this.cargando = false; }, error: () => this.cargando = false }); 
      }

  badgeClase(tipo: string) { 
    return tipo === "VIP" ? "badge-vip" : tipo === "PREMIUM" ? "badge-premium" : "badge-pendiente"; }
    ...
    .then(r => { if (r.isConfirmed) this.svc.eliminar(c.id!).subscribe(() => { 
