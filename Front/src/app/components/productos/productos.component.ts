...
export class ProductosComponent implements OnInit {
  lista: Producto[] = []; cargando = false;
  constructor(private svc: ProductoService) {}
  ngOnInit() { this.cargar(); }
  cargar() { this.cargando = true; this.svc.listar().subscribe({ next: d => { this.lista = d as Producto[]; this.cargando = false; }, error: () => this.cargando = false }); }
  eliminar(p: Producto) {
